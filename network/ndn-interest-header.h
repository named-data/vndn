 ...
 *   <xs:simpleContent>
 *     <xs:extension base="Binarytime_duration12">
 *       <xs:attribute name="ccnbencoding" type="xs:string" fixed="base64Binary"/>
 *     </xs:extension>
 *   </xs:simpleContent>
 * </xs:complexType>
 *
 * <xs:simpleType name="Binarytime_duration12">
...
public:
     ...
     * @param[in] name const pointer to NameComponents object that contains an interest name
     **/
    void
     ...
     *
     * For example, SetExclude (NameComponents("exclude1")("exclude2")("exclude3"))
     * @param[in] exclude const pointer to NameComponents to be excluded
     **/
    void
     ...
     */
    void
    SetInterestLifetime (const time_duration &time);
    ...
    */
     ...
     */
    virtual uint32_t Deserialize (const Buffer &buffer);
...
private:
    ...
    time_duration  m_interestLifetime;           ///< InterestLifetime
    uint32_t m_nonce;                   ///< Nonce. not used if zero
    uint32_t m_nackType;                ///< Negative Acknowledgement type
};

/**
 * \brief Exception class for InterestHeader
 */
