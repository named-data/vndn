...
size_t
NDNEncodingHelper::Serialize (Buffer &start, const InterestHeader &interest)
{
    size_t written = 0;
    ...
    if (interest.GetInterestLifetime() != seconds(0)) {
...
size_t
NDNEncodingHelper::GetSize (const InterestHeader &interest)
{
    size_t written = 0;
...
size_t
NDNEncodingHelper::Serialize (Buffer &start, const ContentObjectHeader &contentObject)
{
    size_t written = 0;
...
size_t
NDNEncodingHelper::GetSize (const ContentObjectHeader &contentObject)
{
    size_t written = 0;
...
size_t
NDNEncodingHelper::AppendBlockHeader (Buffer &start, size_t val, CcnbParser::ccn_tt tt)
{
...
size_t
NDNEncodingHelper::AppendNumber (Buffer &start, uint32_t number)
{
...
size_t
NDNEncodingHelper::AppendCloser (Buffer &start)
{
...
size_t
NDNEncodingHelper::AppendNameComponents (Buffer &start, const NameComponents &name)
{
    size_t written = 0;
...
size_t
NDNEncodingHelper::EstimateNameComponents (const NameComponents &name)
{
    size_t written = 0;
    BOOST_FOREACH (const std::string & component, name.GetComponents()) {
...
size_t
NDNEncodingHelper::AppendTimestampBlob (Buffer &start, const time_duration &time)
{
...
size_t
NDNEncodingHelper::EstimateTimestampBlob (const time_duration &time)
{
    int required_bytes = 2; // 12 bits for fractions of a second, 4 bits left for seconds. Sometimes it is enough
...
size_t
NDNEncodingHelper::AppendTaggedBlob (Buffer &start, CcnbParser::ccn_dtag dtag,
